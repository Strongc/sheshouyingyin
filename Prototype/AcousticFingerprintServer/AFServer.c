...
recieve_msg(receiver, &msg_size, &more, &more_size, &data); \
			...
			{ \
				printf("SIZE not fit msg_size:%d, size:%d",msg_size,SIZE); \
...
static int pull_message(int thrn, void *receiver)
{
	...
	RECEIVEMSG(sizeof(uint8_t),&m_phashframe.earlyendflag,MISSINGEARLYENDFLAG);
	...
	RECEIVEMSG(sizeof(uint8_t),&m_phashframe.amount,MISSINGAMOUNT);
	...
	LOG(log);
	// pull id msg part
	RECEIVEMSG(sizeof(uint8_t),&m_phashframe.id,MISSINGID);
	...
	RECEIVEMSG(sizeof(uint32_t),&m_phashframe.nbframes,MISSINGNBFRAMES);
	...
	LOG(log);
	// pull hash msg part 
    recieve_msg(receiver, &msg_size, &more, &more_size, (void **)&m_phashframe.phash);
